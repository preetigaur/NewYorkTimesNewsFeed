...
class NewsFeedsListVC: UIViewController {
  ...
  func setUpView() {
    ...
    self.tableView.register(UINib.init(nibName: "NewsFeedCell", bundle: nil), forCellReuseIdentifier: "NewsFeedCell")
...
extension NewsFeedsListVC: UITableViewDelegate, UITableViewDataSource {
    ...
    if media.count > 0 {
      ...
      if filteredMediaMetadata.count > 0 {
        ...
        imageLoader.downloadImageWithPath(imagePath: filteredMediaMetadata[0].url) { (image) in
          if let updateCell = tableView.cellForRow(at: indexPath) as? NewsFeedCell {
  ...
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    let feedWebVC = storyboard.instantiateViewController(withIdentifier: "FeedWebVC") as! NewsFeedWebVC
